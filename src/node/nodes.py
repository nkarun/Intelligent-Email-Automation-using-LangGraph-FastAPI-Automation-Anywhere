...
from src.llms.agents import Agents
...
class Nodes:
    ...
    def Intialize_new_emails(self, state: GraphState) -> GraphState:
        subject1 = state.get("subject", "")
        body1 = state.get("body", "")
        email_obj= Email(
        id="1",
        threadId="thread-1",
        messageId="msg-1",
        ...
        if "emails" not in state or state["emails"] is None:
            state["emails"] = []
        state["emails"].append(email_obj)
        state["current_email"] = email_obj
        print(f"Initialized Email:\nSubject: {email_obj.subject}\nBody: {email_obj.body}\n")
        return state
    ...
    def create_draft_response(self, state: GraphState) -> GraphState:
        ...
        return {"data": state["generated_email"]}
    ...
    def send_email_response(self, state: GraphState) -> GraphState:
        ...
        return {"retrieved_documents": "", "trials": 0, "current_email": state["current_email"], "generated_email": state["generated_email"]}
